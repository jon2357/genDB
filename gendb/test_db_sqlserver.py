...
def test_sql_query_return_1_result_pass_var_on_init():
    sql = "SELECT TOP (?) * FROM testTable WHERE 1 = 1 "
    inVars = 1
...
def test_sql_query_add_conditional_integer_field_search_integer():
    ...
    str2match = "AND ISNUMERIC(testInt) = 1 AND TRY_CONVERT(bigint, testInt) != ? AND ISNUMERIC(testIntNull) = 1 AND TRY_CONVERT(bigint, testIntNull) < ?"
...
def test_sql_query_add_conditional_string_field_search_int():
    ...
    str2match = "AND ISNUMERIC(testVarChar) = 1 AND TRY_CONVERT(bigint, testVarChar) = ?"
    assert (db.sql.find(str2match) == -1) == False
    assert db.vars == [992]
    assert len(db.result) == 2
